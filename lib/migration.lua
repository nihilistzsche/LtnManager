-- Vendored from flib <= 0.16.x. The `ltnm_migration` module was removed in
...
--- @class ltnm_migration
local ltnm_migration = {}
...
--- @return string?
function ltnm_migration.format_version(version, format)
  if version then
...
--- @param format string? default: `%02d`
--- @return boolean?
function ltnm_migration.is_newer_version(old_version, current_version, format)
  local v1 = ltnm_migration.format_version(old_version, format)
  local v2 = ltnm_migration.format_version(current_version, format)
  if v1 and v2 then
    if v2 > v1 then
      return true
    end
...
function ltnm_migration.run(old_version, migrations, format, ...)
  local migrate = false
  for version, func in pairs(migrations) do
    if migrate or ltnm_migration.is_newer_version(old_version, version, format) then
      migrate = true
      func(...)
    end
  end
...
function ltnm_migration.on_config_changed(e, migrations, mod_name, ...)
  local changes = e.mod_changes[mod_name or script.mod_name]
  local old_version = changes and changes.old_version
  if old_version then
    if migrations then
      ltnm_migration.run(old_version, migrations, nil, ...)
    end
    return true
  end
  return false
...
function ltnm_migration.handle_on_configuration_changed(version_migrations, generic_handler)
  script.on_configuration_changed(function(e)
    if ltnm_migration.on_config_changed(e, version_migrations) and generic_handler then
      generic_handler(e)
    end
  end)
end

return ltnm_migration
