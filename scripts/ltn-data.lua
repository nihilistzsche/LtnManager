...
local function update_history(working_data)
  ...
  for _, entry in pairs(active_data.history_to_add) do
    ...
    if not train or not train.to then
      -- check for `data`
      if entry.use_working_data and not storage.data then
        goto continue
      end
      -- try other table
...
local function update_alerts(working_data)
  ...
  for _, alert_data in pairs(active_data.alerts_to_add) do
    ...
    if not train or not train.to then
      -- check for `data`
      if alert_data.use_working_data and not storage.data then
        goto continue
      end
      -- try other table
...
function ltn_data.on_stops_updated(e)
  if storage.flags.iterating_ltn_data or storage.flags.updating_guis then
    return
  end
